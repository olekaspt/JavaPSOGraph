...
public class CreateOptimizedHeuristicGraphs1a_hmm {
...
//	int MaxEdges = 936;
	...
	int NodeLimit = 40 ; ///8;//13;//15;
	
	int HubLimit = 58; //16
	double perIntial = .50;
	
	double hubConnectPer = 0.0168 * ((double)2/(double)5);
	...
	String OutDir0 = BaseDir0 + "\\optimizedheuristicHmm";
	...
	public static void main(String[] args)
	{
		try
		{
			CreateOptimizedHeuristicGraphs1a_hmm createOptimizedHeuristicGraphs = new CreateOptimizedHeuristicGraphs1a_hmm();
	...
	public CreateOptimizedHeuristicGraphs1a_hmm()
	{
		r = new Random();
...
//		SeedDir.add(SeedDir11);
	...
	private void run() throws Exception
	{
		...
		System.out.println("HubLimit: "+ HubLimit);
		System.out.println("perIntial: "+ perIntial);
		System.out.println("hubConnectPer: "+ hubConnectPer);
...
//				PercentageInLargestCluster lcc ;
				...
				for(int j =0; j < vNodes.size(); j++)
				{
					...
					System.out.println("numToAccept: "	+numToAccept+"intNumToAccept: "+intNumToAccept);
						//(int)numToAccept;
...
//				System.out.println(perLCC+" perLCC");
			...
			//	System.out.println("High Degree Node total "+highDegreenodes.size());
				...
				for(int j=0; j < highDegreenodes.size(); j++)
				{
					...
					for(int k =0; k < candNodes.size(); k++)
					{
						...
						if(Double.compare(per,hubConnectPer) > 0)
							continue;
			...
			//	System.out.println("High Degree Node total "+highDegreenodes.size());
				...
				{
					System.out.println("Hubs High degree iter "+j);
					//Connect to another high degree node if within .5
...
//
				...
				mout.write("\n");
				mout.write("	Number of edges greater than  .16 -" + calcGraph.getNumberLongEdges(.16) );
				mout.write("\n");
				mout.write("	Number of edges less or equal to .16 - " + calcGraph.getNumberShortEdges(.16));
				mout.write("\n");
