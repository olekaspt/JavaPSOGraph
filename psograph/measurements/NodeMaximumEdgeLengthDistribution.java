...
{
	public NodeMaximumEdgeLengthDistribution(Graph g) 
	{
		m_graph = g;
		isCalculated = false;
	}
	...
	public TreeMap<Double, Integer> Measure() throws Exception 
	{
		...
		for(int i = 0; i < m_graph.getNumberOfNodes(); i++)
		{
			...
			double maxEdgeCLengthForNode = -1;
			...
			for(int j = 0; j < m_graph.getNumberOfNodes(); j++)
			{
				if(adjMatrix[i][j]== 1)
				{
					...
					if(Double.compare(distance, maxEdgeCLengthForNode) > 0.0)
					{
						maxEdgeCLengthForNode = distance;
					}
				}
			}

			
			m_resultById.put(node_i_id, maxEdgeCLengthForNode);
